...
int	get_next_line(int fd, char **line)
{
	...
	if (ft_error(fd, buffer, charsread))
		return (-1);
	while (nl_check(leftchars[fd]) && charsread != 0)
	{
		charsread = read(fd, buffer, BUFFER_SIZE);
		if (ft_error(fd, buffer, charsread))
			return (-1);
		buffer[charsread] = '\0';
		leftchars[fd] = strjoin_free(leftchars[fd], buffer);
		if (!leftchars[fd])
			return (-1);
	}
	free (buffer);
	*line = ft_line(leftchars[fd]);
	leftchars[fd] = ft_leftchars(leftchars[fd]);
